...
for i in $(seq 1 60); do H=$(curl -s --max-time 2 http://127.0.0.1:5199/api/stats | python3 -c "import json,sys;print(json.load(sys.stdin).get('network_height'))" 2>/dev/null); [ "$H" = "49" ] && break; sleep 1; done
echo "clone tip after mining: $H"; curl -s http://127.0.0.1:5199/api/stats > $R/tip-after-F.json
...
for i in $(seq 1 120); do HB=$(curl -s --max-time 2 http://127.0.0.1:5198/api/stats | python3 -c "import json,sys;print(json.load(sys.stdin).get('network_height'))" 2>/dev/null); [ "$HB" = "49" ] && break; sleep 2; done
...
keys=["tip","state_root","balances","token_balances","lp_balances","burned_tokens","stakes","shielded_supply_bits","base_fee_quanta","validators","total_stake","quorum","unbonding","fees_burned_bits"]  # nonce_db excluded: never read by block import; cleared by the pre-existing startup heuristic (finding)
...
ok = proj(A)==proj(A2)==proj(B) and A.get("tip")==49 and A.get("total_stake")==110000 and A.get("quorum")==73334 and A.get("unbonding")==[]
